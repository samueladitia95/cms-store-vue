<template>
  <div>
    <!-- Black Shadow -->
    <div
      class="bg-black opacity-40 fixed h-full w-screen top-0 left-0 md:hidden z-10"
      :class="{ 'hidden': sidebarClass }"
      @click="toggleSidebar"
    ></div>

    <div
      class="bg-gray-50 min-h-full w-72 h-1 z-40 overflow-y-auto transform transition duration-200 ease-in-out fixed top-0 md:static md:transform-none"
      :class="{ '-translate-x-full': sidebarClass }"
    >
      <div class="flex flex-col min-h-full w-72 md:fixed md:top-0 p-4">
        <!-- Header -->
